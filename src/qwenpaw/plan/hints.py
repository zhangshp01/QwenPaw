...
_GOV_DOC_PIPELINE_NO_PLAN = (
    ...
    "**规划规则**：按下列顺序 **只选一类**；子任务数 = 该类上限，"
    "**禁止**跨类拼工具或「多做事」。子任务 name/description 须能让执行步"
    "对应唯一工具（检索/起草/审核/版式）。\n\n"
    "【1 窄 · 仅审核】用户已贴正文，或含「如下信息/如下内容/原文/请审核」等：\n"
    "   → `create_plan` **1 个子任务**，工具 **仅** `doc_reviewer`。\n"
    "   **禁止** `doc_retrieval`、`gov_document_writer`、`gov_document_layout`。\n\n"
    "【2 窄 · 仅版式】仅要模板/排版/版式推荐（无起草、无审核诉求）：\n"
    "   → **1 个子任务**，工具 **仅** `gov_document_layout`（执行时 **只**传 "
    "``template_title``）。**禁止**检索、起草、审核。\n\n"
    "【3 窄 · 仅检索】仅找资料/检索知识库，未要求写文或排版：\n"
    "   → **1 个子任务**，工具 **仅** `doc_retrieval`；items 到手后 **即** "
    "`finish_plan`。**禁止**自动起草、审核、版式。\n\n"
    "【4 宽 · 完整正式文稿】命中 **任一** 即属本类（且不属于 1–3）：\n"
    "   · 用户要「写一篇/帮我写/拟一份/生成」+（公文|通知|函|报告|方案|文件）\n"
    "   · 或要求「完整公文/正式文稿/全套流程」\n"
    "   → `create_plan` **必须 4 个子任务**（不可省略检索直接写作），顺序固定：\n"
    "   ① 检索（`doc_retrieval`）→ ② 起草（`gov_document_writer`）→ "
    "   ③ 审核（`doc_reviewer`）→ ④ 版式（`gov_document_layout`，仅 "
    "``template_title``）。\n"
    "   除非用户原话含「不要检索/跳过检索/无需查资料/已有材料」，否则 **不得** "
    "建 3 步 writer→reviewer→layout。\n"
    "   每步只调用对应工具；诉求已满足则 `finish_plan`，勿自动加步。\n\n"
    "`create_plan` 成功后 **不要** 请用户确认；**立即** `update_subtask_state`，"
    "subtask_idx=0、state='in_progress'，本回合开始执行第一步。\n"
)
...
_GOV_DOC_PIPELINE_NO_PLAN_SKIP_RETRIEVAL = (
    ...
    + _LANG_BLOCK
    + "你处于 **/plan 公文助手**（自动执行；无需向用户确认）。\n"
    "**用户已提供参考材料**（附件与/或正文线索），**禁止**为取材料调用 "
    "`doc_retrieval`。\n"
    "窄场景 1–3 同常规 /plan（仅审核 / 仅版式 / 仅检索，各 1 子任务）。\n"
    "【宽 · 完整正式文稿】用户要写完整公文且不属于窄场景时：\n"
    "   → **3 或 4 个子任务**（**跳过检索**）：\n"
    "   ① 消化材料（**不**调 `doc_retrieval`，`finish_subtask` 简述要点）→ "
    "   ② `gov_document_writer` → ③ `doc_reviewer` → ④ `gov_document_layout`。\n"
    "   若用户仅要正文+审核、未要版式，可 3 步（末步为 reviewer 后 `finish_plan`）。\n"
    "   诉求已满足即 `finish_plan`，勿加计划外工具。\n\n"
